...
public class Binary
{
	...
	*
	* @param num1 The first operand object (multiplicand)
	* @param num2 The second operand object (multiplier)
	* @return A binary variable with a value of <i>num1 * num2</i>.
	*/
	public static Binary multiply(Binary num1, Binary num2) 
	{
		...
		while (ind2 >= 0) {
			if (num2.number.charAt(ind2) == '1') {
				// Shift num1 left by the current shiftCounter
				String shiftedVal = num1.number;
				for (int i = 0; i < shiftCounter; i++) {
					shiftedVal += "0";
				}
				// Add the shifted value to the running total
				result = add(result, new Binary(shiftedVal));
			}
			ind2--;
			shiftCounter++;
		}
